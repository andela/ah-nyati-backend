...
describe('Testing Password Reset Controller', () => {
  describe('Testing password reset controller', () => {
    const passwordResetAuth = '/api/auth/';
    it(
      ...
      async () => {
        ...
        expect(response.body.data).to.equal('reset code successfully sent to email');
      },
    );
    ...
    it(
      ...
      async () => {
        ...
        expect(response).to.be.an('object');
        expect(response).to.have.status(200);
        expect(response.body.data).to.equal('reset code resent to your email');
      },
    );
    ...
    it(
      ...
      async () => {
        ...
        expect(response.body.data).to.equal('token has expired');
      },
    );
  });
});
